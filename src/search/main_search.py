...
## epochs
parser.add_argument('--epochs_srk', type=int, default=200, help='num of training epochs for each shrink and drop')
parser.add_argument('--epochs_nspa', type=int, default=1000, help="shrinked new space re-search")
parser.add_argument('--epochs_tau_srk', type=int, default=0, help="shrink space fix tau")
...
## shrink
parser.add_argument('--num_to_keep', nargs='+', type=int,
                    help='[15,12,9], has to be less than the total number 18, and length > 1')
parser.add_argument('--num_to_drop', nargs='+', type=int, help='[3,3,3]')
parser.add_argument('--eps_no_archs', nargs='+', type=int, help='[0,0,0]')
...
def main():
    ...
    # 6. starting progressive heterophily aware search space shirinking

    for drop_nd in range(len(num_to_keep)):
        model = Network(criterion, dataset.num_features, dataset.num_classes, hidden_size, device,
                        switches_na=switches_na,
                        args=args)
        model = model.to(device)
        logging.info("param size = {}MB".format(src.utils_lib.utils.count_parameters_in_MB(model)))
        network_params = []
        ...
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
            optimizer, float(args.epochs_srk), eta_min=args.learning_rate_min)
        arch_lr = args.arch_learning_rate / (drop_nd + 1)
        eps_no_arch = eps_no_archs[drop_nd]
        search_cost = 0
        for epoch in range(args.epochs_srk + args.epochs_tau_srk):
            lr = scheduler.get_last_lr()[0]
            logging.info('Epoch:{}, lr: {:.6f}, arch_lr:{:.6f}'.format(epoch, lr, arch_lr))
            epoch_start = time.time()
            # training
            # single time set tau
            if epoch < args.epochs_srk:
                model.set_tau(args.tau_max - (args.tau_max - args.tau_min) * epoch / (args.epochs_srk - 1))
            else:
                model.set_tau(args.tau_min)

            if epoch < eps_no_arch:
                train_acc, train_obj, arch_obj = train_shrink(epoch, data, model, network_params, criterion, optimizer,
                                                              lr, arch_lr,
                                                              train_arch=False)
            else:
                train_acc, train_obj, arch_obj, _ = train_shrink(epoch, data, model, network_params, criterion,
                                                                 optimizer, lr, arch_lr,
                                                                 train_arch=True)
            search_cost += (time.time() - epoch_start)
            valid_acc, valid_obj = infer_trans(epoch, args.data, data, model, criterion)
            test_acc, test_obj = infer_trans(epoch, args.data, data, model, criterion, test=True)
            scheduler.step()
            logging.info('Epoch={}, Tau={:.04f}, '
                         'train_acc={:.04f}, valid_acc={:.04f}, test_acc={:.04f}, '
                         'arch_loss = {:.04f},train_loss={:.04f}, valid_loss={:.04f}, test_loss={:.04f}'
                         .format(epoch, model.get_tau(),
                                 train_acc, valid_acc, test_acc,
                                 arch_obj, train_obj, valid_obj, test_obj))
            writer.add_scalar('Drop/Train/loss_' + str(args.seed) + '_' + str(drop_nd), train_obj, epoch)
            writer.add_scalar('Drop/Valid/loss_' + str(args.seed) + '_' + str(drop_nd), valid_obj, epoch)
            writer.add_scalar('Drop/Test/loss_' + str(args.seed) + '_' + str(drop_nd), test_obj, epoch)
            writer.add_scalar('Drop/Arch_val/loss_' + str(args.seed) + '_' + str(drop_nd), arch_obj, epoch)
            writer.add_scalar('Drop/Train/acc_' + str(args.seed) + '_' + str(drop_nd), train_acc, epoch)
            writer.add_scalar('Drop/Valid/acc_' + str(args.seed) + '_' + str(drop_nd), valid_acc, epoch)
            writer.add_scalar('Drop/Test/acc_' + str(args.seed) + '_' + str(drop_nd), test_acc, epoch)

        logging.info('arch_param_magnitude:{}'.format(F.softmax(model.arch_parameters()[0], dim=-1)))
        src.utils_lib.utils.save(model, os.path.join(log_dir, 'weights.pt'))
        logging.info('------Dropping {}-th rounds with #{}_ops ------'.format(drop_nd, num_to_drop[drop_nd]))
        arch_param = model.arch_parameters()
        na_prob = F.softmax(arch_param[0], dim=-1).data.cpu().numpy()
        for i in range(3):
            idxs = []
            for j in range(len(PRIMITIVES)):
                if switches_na[i][j]:
                    idxs.append(j)
            drop = get_min_k(na_prob[i, :], num_to_drop[drop_nd])
            for idx in drop:
                switches_na[i][idxs[idx]] = False
        logging.info('switches_na = %s', switches_na)
        logging_switches(switches_na, PRIMITIVES)
    logging.info('Finish the space shrink...')

    # 6. searching few epochs on the shrinked search space
    ...
    for epoch in range(args.epochs_nspa + args.epochs_tau_nspa):
        ...
        scheduler_new.step()
        logging.info('Shrinked Training Epoch={}, Tau={:.04f}, '
                     'train_acc={:.04f}, valid_acc={:.04f}, test_acc={:.04f},'
                     'arch_loss = {:.04f}, train_loss={:.04f}, valid_loss={:.04f}, test_loss={:.04f}'
        ...
        writer.add_scalar('Shrinked/Train/loss_' + str(args.seed), train_obj, epoch)
        writer.add_scalar('Shrinked/Valid/loss_' + str(args.seed), valid_obj, epoch)
        writer.add_scalar('Shrinked/Test/loss_' + str(args.seed), test_obj, epoch)
        writer.add_scalar('Shrinked/Arch_val/loss_' + str(args.seed), arch_obj, epoch)
        writer.add_scalar('Shrinked/Train/acc_' + str(args.seed), train_acc, epoch)
        writer.add_scalar('Shrinked/Valid/acc_' + str(args.seed), valid_acc, epoch)
        writer.add_scalar('Shrinked/Test/acc_' + str(args.seed), test_acc, epoch)
    ...
    if args.resume_cpk == '':
        ...
        argmax_geno_vloss = model_new.genotype()
        argmax_geno.append(argmax_geno_vloss)

        logging.info('Using valid loss crit for arch selection...')
        args.proj_crit_NA, args.proj_crit_SC, args.proj_crit_SC = 'loss', 'loss', 'loss'
        model_new._initialize_flags()
        geno_out_loss_vloss = pt_project(data, model_new, criterion, device, args)
        geno_out_loss.append(geno_out_loss_vloss)
    ...
    return argmax_geno, geno_out_loss, geno_out_hetro
...
def run_by_seed():
    ...
    for i in range(args.search_num):
        ...
        args.seed = seed
        argmax_geno, geno_out_loss, geno_out_hetro = main()

        res_vloss.append(
            'VLOSS_seed={}, argmax_geno={}, proj_genotype_loss={}, proj_genotype_hetro={}, saved_dir={}'.format(
                args.seed, argmax_geno[0], geno_out_loss[0], geno_out_hetro[0], log_dir))
